...
let shownNetworks: ComputedRef<INetwork[]> = computed(() => {
  ...
  priorities.sort(
    (
      a: { id: string; priorty: number },
      b: { id: string; priorty: number }
    ) => {
  ...
  if (priorities.length != 0) {
    ...
    priorities = priorities.filter(
      (item) =>
        item.priorty == maxPri || item.priorty >= kAlwaysShowMinPriority_
    );
  }
...
<template>
  <q-page padding>
    <div class="row text-h4 text-center q-mb-md">
      <div class="col">Why The Fuck is My Network So Slow?</div>
    </div>
