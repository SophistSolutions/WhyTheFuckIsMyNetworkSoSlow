...
import DeviceDetails from 'src/components/DeviceDetails.vue';
  ...
  allDevices.value.forEach((i: IDevice) => {
    ...
    if (passedFilter && selectedTimeframe.value !== null) {
      if (i.seen?.Ever?.upperBound == null) {
        ...
        const selectedTimeframeAsMS = Duration.fromISO(selectedTimeframe.value).as('milliseconds');
            ...
            <q-tr :props="props">
              ...
              <q-td :props="props" key="lastSeenAt">
                <ReadOnlyTextWithHover v-if="props.row.seen?.Ever?.upperBound"
                  :message="DateTime.fromJSDate(props.row.seen.Ever.upperBound).toRelative()" />
              </q-td>
