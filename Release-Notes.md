...
- Backend
  ...
  - Refactoring 
    - Major restructuring of IntegrationModel code, separating concerns into dedicated files for DBAccess, RolledUpNetworks, RolledUpDevices, and RolledUpNetworkInterfaces.
  - Rollup Logic
    - **mostly new**
    - IntegratedModel::Mgr (where we do rollups)
      - Combines database values, discovered values, and synthesizes rolled up version
    - Network
      - Extensive refactoring of network rollup functionality, including fingerprinting algorithms, user settings integration,
        and efficient re-rollup handling. Introduced GenerateFingerprintFromProperties() method and cleaned up rollup logic.
    - Network Interface
      - Implementation of network interface aggregation similar to devices/networks. 
      - Added UI components for displaying network interface details, including attached devices and seen information.
  ...
  - https://github.com/SophistSolutions/WhyTheFuckIsMyNetworkSoSlow/issues/38 - Common::PrioritizedNames support 
  - Database
    - Migration to new DocumentDB API
    - support for multiple storage backends (SQLite, JSON files)
...
- Build Infrastructure & Docker 
  - Expanded Docker support for Ubuntu 22.04/24.04 and Windows builds
  - GitHub Actions workflows for continuous integration on Windows, Linux, and macOS platforms.
- HTMLUI
  - Show new Model features (like network address, name)
  - tons of cleanups 
  - Support showing snapshots and rolled up logic, and links between them
  - NetworkInterface 
    - code showing Seen range and showing it in breadcrumbs
    - format baud rate
  - Added network/device name editing with popup text fields
  - added tags support for devices
  - breadcrumb navigation improvements
  - context menus
  - filtering options (old networks, inactive interfaces).
  - Replaced MomentJS with Luxon for date/time handling
  - enhanced About page with API call tracking
- Model
  ...
  - Seen
    - re-engineer date range mapper code for Seen - so uses kDateRangeMapper_ instead of global range mapper (lower-camel-case names happier in javascript)
  ...
  - Network
    - gateways and dns fields of networks now sets; 
    - new fGatewayHardwareAddresses field of network
  - NetworkInterface
  - UserSettings
    - Tags: https://github.com/SophistSolutions/WhyTheFuckIsMyNetworkSoSlow/issues/39
      ...
      - includes logic in backend and API to rollup
